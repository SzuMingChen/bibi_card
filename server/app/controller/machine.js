const { reader_machine } = require('../model/machine');
const {checkInOut} =require('./attendance')
...
function sendTarget(postData,io) {
    ...
    for (let i = 0; i < targetList.length; i++) {
        switch (targetList[i]) {
            case 'admin':
                console.log("admin");
                io.emit("get-card-id",  send({ code: "0001", msg: "成功"}, postData));

                break;
                
                case 'user':
                    console.log("user");
                    checkInOut(postData,(responseStatus, data, message)=>{
                        console.log("Inswitch",send(responseStatus, postData));
                        if (responseStatus.code !== "0001") return;
...
exports.barcodeMessagePackage = async (req, res, io) => {
